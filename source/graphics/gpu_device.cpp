...
//    }
    ...
    if ( dynamic_rendering_extension_present ) {
        cmd_begin_rendering = ( PFN_vkCmdBeginRendering )vkGetDeviceProcAddr( vulkan_device, "vkCmdBeginRendering" );
...
SamplerHandle GpuDevice::create_sampler( const SamplerCreation& creation ) {
    ...
    if ( creation.reduction_mode != VK_SAMPLER_REDUCTION_MODE_WEIGHTED_AVERAGE ) {
...
DescriptorSetLayoutHandle GpuDevice::create_descriptor_set_layout( const DescriptorSetLayoutCreation& creation ) {
    ...
    if ( creation.bindless ) {
        ...
        // Binding flags
        VkDescriptorBindingFlags bindless_flags = VK_DESCRIPTOR_BINDING_PARTIALLY_BOUND_BIT | VK_DESCRIPTOR_BINDING_UPDATE_AFTER_BIND_BIT;//VK_DESCRIPTOR_BINDING_VARIABLE_DESCRIPTOR_COUNT_BIT
...
void GpuDevice::create_swapchain() {
    ...
    if ( synchronization2_extension_present ) {
        ...
        check( queue_submit2( vulkan_main_queue, 1, &submit_info, VK_NULL_HANDLE ) );
    } else {
        VkSubmitInfo submitInfo = { VK_STRUCTURE_TYPE_SUBMIT_INFO };
        submitInfo.commandBufferCount = 1;
        submitInfo.pCommandBuffers = &command_buffer->vk_command_buffer;
...
void GpuDevice::present( CommandBuffer* async_compute_command_buffer ) {
    ...
    if ( timeline_semaphore_extension_present ) {
        ...
        if ( synchronization2_extension_present ) {
            ...
            check( queue_submit2( vulkan_main_queue, 1, &submit_info, render_complete_fence ) );
